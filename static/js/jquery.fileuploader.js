...
(function($) {
	...
	$.fn.fileuploader.defaults = {
        ...
        captions: {
            ...
            removeConfirmation: '确定要删除此附件吗？',
            errors: {
                filesLimit: '附件不能超过 ${limit} 个。',
                filesType: '只能上传 ${extensions} 格式的附件。',
...
export function initFileuploader () {
  var uploadInputName="files";
  $('input[name="'+ uploadInputName + '"]').fileuploader({
  extensions : ['jpg', 'png', 'jpge', 'gif', 'doc', 'docx', 'xls', 'xlsx', 'pdf', 'txt', 'ppt', ''],
  changeInput: ' ',
      theme: 'thumbnails',
      enableApi: true,
      addMore: true,
      UploaderType : 'default', //上传类型 default:单文件 multi:多文件
      uploaderInput : 'title',//文件名
      uploaderUrl:'',
      thumbnails: {
          box: '<div class="fileuploader-items">' +
          ...
          item: '<li class="fileuploader-item">' +
                     '<div class="fileuploader-item-inner" >' +
                         '<div class="thumbnail-holder">${image}</div>' +
                           '<div class="fileuploader-item-select glyphicon glyphicon-ok"></div>' +
          ...
          _selectors: {
              ...
              retry: '.fileuploader-action-retry',
              remove: '.fileuploader-action-remove',
              selected: '.fileuploader-item-select'
          },
...
//            alert('1');
      ...
      //    var uploadFilename = $('#title').val();
      //    var newFileName='';
      //    var uploadFilename = uploadFilename.split(';');
      //    for (var i = 0, l = uploadFilename.length; i < l; i++) {
      //     var filename = $.trim(uploadFilename[i]);
